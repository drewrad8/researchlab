...
Worker ID: f8d517af | Label: IMPL: resume stalled endocrine disruptors research + triage NYC skincare | Role: Specialist Worker
...
Report back: `curl -s -X POST http://localhost:38007/api/workers/dfb2c0ce/input -H "Content-Type: application/json" -d '{"input":"your message","fromWorkerId":"f8d517af"}'`
</parent>

## Ralph Signaling (Worker ID: f8d517af)

Signal progress regularly so your commander knows you're alive:
```bash
curl -s -X POST http://localhost:38007/api/ralph/signal/by-worker/f8d517af -H "Content-Type: application/json" -d '{"status":"in_progress","progress":50,"currentStep":"describing what you are doing now"}'
```
...
| My siblings | GET | `/api/workers/f8d517af/siblings` |
| My children | GET | `/api/workers/f8d517af/children` |
...
Spawn body: `{"template":"TYPE","label":"NAME","projectPath":"/home/druzy/thea/researchlab-release","parentWorkerId":"f8d517af","task":{"description":"..."}}`
...
**Spawn >60s tasks. Check siblings first. Include parentWorkerId: "f8d517af" in ALL spawns.**

## Work Practices

- Git commit frequently. Uncommitted work is LOST when workers are terminated.
- Check running siblings before spawning: `curl -s http://localhost:38007/api/workers/f8d517af/siblings`
