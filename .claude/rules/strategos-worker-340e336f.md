...
Worker ID: 340e336f | Label: RESEARCH: P-TEC L3 "CLUSTER INVESTIGATION [undefined-TEC]: 6 TEC items for undefined. Items: [1] e0-018: No description (source: ?, info: ?)" | Role: Specialist Worker
...
## Ralph Signaling (Worker ID: 340e336f)

Signal progress regularly so your commander knows you're alive:
```bash
curl -s -X POST http://localhost:38007/api/ralph/signal/by-worker/340e336f -H "Content-Type: application/json" -d '{"status":"in_progress","progress":50,"currentStep":"describing what you are doing now"}'
```
...
| My siblings | GET | `/api/workers/340e336f/siblings` |
| My children | GET | `/api/workers/340e336f/children` |
...
Spawn body: `{"template":"TYPE","label":"NAME","projectPath":"/home/druzy/thea/researchlab-release","parentWorkerId":"340e336f","task":{"description":"..."}}`
...
**Spawn >60s tasks. Check siblings first. Include parentWorkerId: "340e336f" in ALL spawns.**

## Work Practices

- Git commit frequently. Uncommitted work is LOST when workers are terminated.
- Check running siblings before spawning: `curl -s http://localhost:38007/api/workers/340e336f/siblings`
