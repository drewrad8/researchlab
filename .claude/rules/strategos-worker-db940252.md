...
Worker ID: db940252 | Label: RESEARCH: P-GOV L3 "CLUSTER INVESTIGATION [undefined-GOV]: 16 GOV items for undefined. Items: [1] e0-006: No description (source: ?, info: ?" | Role: Specialist Worker
...
## Ralph Signaling (Worker ID: db940252)

Signal progress regularly so your commander knows you're alive:
```bash
curl -s -X POST http://localhost:38007/api/ralph/signal/by-worker/db940252 -H "Content-Type: application/json" -d '{"status":"in_progress","progress":50,"currentStep":"describing what you are doing now"}'
```
...
| My siblings | GET | `/api/workers/db940252/siblings` |
| My children | GET | `/api/workers/db940252/children` |
...
Spawn body: `{"template":"TYPE","label":"NAME","projectPath":"/home/druzy/thea/researchlab-release","parentWorkerId":"db940252","task":{"description":"..."}}`
...
**Spawn >60s tasks. Check siblings first. Include parentWorkerId: "db940252" in ALL spawns.**

## Work Practices

- Git commit frequently. Uncommitted work is LOST when workers are terminated.
- Check running siblings before spawning: `curl -s http://localhost:38007/api/workers/db940252/siblings`
