...
/**
 * Check if a project is paused. If so, update status and throw to halt the pipeline.
 */
function checkPaused(projectId) {
  const current = store.get(projectId);
  if (current && current.paused) {
    store.update(projectId, { status: 'paused', statusDetail: 'Pipeline paused by user' });
    throw new Error('Pipeline paused by user');
  }
}
...
function normalizeManifest(manifest) {
  // Normalize manifest-level evidence array name
  if (manifest.evidence && !manifest.evidenceItems) {
    manifest.evidenceItems = manifest.evidence;
  }

  // Normalize manifest-level sub-question array name
  if (manifest.questions && !manifest.subQuestions) {
    manifest.subQuestions = manifest.questions;
  }

  if (Array.isArray(manifest.evidenceItems)) {
    for (const item of manifest.evidenceItems) {
      // Normalize evidence type field
      if (item.type && !item.evidenceType) {
        item.evidenceType = item.type;
      }
      // Normalize sub-question ID (handle both "subQuestion" and "question")
      if (!item.subQuestionId) {
        if (item.subQuestion) item.subQuestionId = item.subQuestion;
        else if (item.question) item.subQuestionId = item.question;
      }
      // Normalize source/info rating field names
      if (item.sourceReliability && !item.sourceRating) {
        item.sourceRating = item.sourceReliability;
      }
      if (item.credibility && !item.infoRating) {
        item.infoRating = String(item.credibility);
      }
      // Normalize flat citation fields into citation object
      if (!item.citation && (item.url || item.title)) {
        item.citation = {
...
async function phaseAdjudicate(project, plan, manifests, pathwayResults, emitEvent) {
  ...
  for (let qi = 0; qi < questions.length; qi++) {
    ...
    for (const manifest of manifests) {
      if (!manifest.evidenceItems) continue;
      const subs = manifest.subQuestions || manifest.questions || [manifest.subQuestionId];
      if (subs.includes(q.id)) {
        qEvidence.push(...manifest.evidenceItems);
      }
    }

    // If manifest-level matching found nothing, try per-item matching
    if (qEvidence.length === 0) {
      for (const manifest of manifests) {
        if (!manifest.evidenceItems) continue;
        for (const item of manifest.evidenceItems) {
          const itemQ = item.subQuestionId || item.question;
          if (itemQ === q.id) {
            qEvidence.push(item);
          }
        }
...
async function start(project, emitEvent) {
  ...
  try {
    ...
    if (err.message === 'Pipeline paused by user') {
      store.update(project.id, { status: 'paused', statusDetail: 'Pipeline paused by user' });
      emitEvent('phase', { phase: 'paused', status: 'paused' });
    } else {
...
module.exports = { start, resume, normalizeManifest };
