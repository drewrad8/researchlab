...
/**
 * Build clusters from evidence manifests, grouped by (subQuestionId + evidenceType).
 * Each cluster contains all related evidence items for batch investigation.
 */
function buildClusters(manifests) {
  const clusterMap = new Map(); // key: "qN-TYPE" -> { items: [], subQuestionId, evidenceType }

  for (const manifest of manifests) {
    if (!manifest.evidenceItems || !Array.isArray(manifest.evidenceItems)) continue;
    const subs = manifest.subQuestions || [manifest.subQuestionId];

    for (const item of manifest.evidenceItems) {
      const type = item.type || 'UNK';
      for (const sub of subs) {
        const key = `${sub}-${type}`;
        if (!clusterMap.has(key)) {
          clusterMap.set(key, { clusterId: key, subQuestionId: sub, evidenceType: type, items: [] });
        }
...
/**
 * Numeric rating for source rating (A=1 best, F=6 worst).
 */
function sourceRatingNumeric(rating) {
  const map = { A: 1, B: 2, C: 3, D: 4, E: 5, F: 6 };
  return map[rating] || 6;
}

/**
 * Numeric rating for info rating (1=best, 6=worst).
 */
function infoRatingNumeric(rating) {
  const n = parseInt(rating, 10);
  return (n >= 1 && n <= 6) ? n : 6;
}

/**
 * Check if an evidence item is high quality (A or B source, 1 or 2 info rating).
 * These items skip investigation and pass directly to adjudication.
 */
function isHighQuality(item) {
  const sr = (item.sourceRating || '').toUpperCase();
  const ir = parseInt(item.infoRating, 10);
  return (sr === 'A' || sr === 'B') && (ir === 1 || ir === 2);
}

/**
 * Prioritize clusters by lowest average source/info rating (C/D items need investigation most).
 * Returns sorted array: worst-rated clusters first.
 */
function prioritizeClusters(clusters) {
  for (const cluster of clusters) {
    let srcSum = 0, infoSum = 0;
    for (const item of cluster.items) {
      srcSum += sourceRatingNumeric(item.sourceRating);
      infoSum += infoRatingNumeric(item.infoRating);
    }
    const count = cluster.items.length;
    cluster.avgSourceRating = srcSum / count;
    cluster.avgInfoRating = infoSum / count;
    // Combined priority score: higher = worse quality = needs investigation more
    cluster.priorityScore = cluster.avgSourceRating + cluster.avgInfoRating;
  }

  // Sort by priority score descending (worst quality first — most needs investigation)
  clusters.sort((a, b) => b.priorityScore - a.priorityScore);
  return clusters;
}

/**
 * Build a synthetic evidence item representing a cluster for pathway execution.
 * The description includes all items in the cluster so the worker sees the full batch.
 */
function buildClusterEvidence(cluster) {
  const itemDescriptions = cluster.items.map(
    (item, i) => `[${i + 1}] ${item.id}: ${item.description || 'No description'} (source: ${item.sourceRating || '?'}, info: ${item.infoRating || '?'})`
  ).join('; ');

  // Use the most common pathway for this evidence type
  const pathwayId = EVIDENCE_TYPE_PATHWAY[cluster.evidenceType] || null;

  return {
    id: `cluster-${cluster.clusterId}`,
    type: cluster.evidenceType,
    triggeredPathway: pathwayId,
    description: `CLUSTER INVESTIGATION [${cluster.clusterId}]: ${cluster.items.length} ${cluster.evidenceType} items for ${cluster.subQuestionId}. Items: ${itemDescriptions}`,
    sourceRating: cluster.items[0].sourceRating,
    infoRating: cluster.items[0].infoRating,
    _cluster: cluster // attach cluster metadata
  };
}

async function runInvestigationPhase(projectDir, manifests, emitEvent, opts) {
  const maxWorkers = opts?.maxWorkers != null ? opts.maxWorkers : 10;
  ...
  // Build clusters grouped by (subQuestionId + evidenceType)
  const allClusters = buildClusters(manifests);

  // Separate clusters where ALL items are high quality (A/B source + 1/2 info)
  const skipClusters = [];
  const investigateClusters = [];
  for (const cluster of allClusters) {
    if (cluster.items.every(isHighQuality)) {
      skipClusters.push(cluster);
    } else {
      investigateClusters.push(cluster);
    }
  }

  // Prioritize: worst-quality clusters first
  prioritizeClusters(investigateClusters);

  // Cap to budget: only investigate up to maxWorkers clusters at L1
  const l1Clusters = investigateClusters.slice(0, maxWorkers);
  const skippedByBudget = investigateClusters.slice(maxWorkers);

  emitEvent('phase', {
    phase: 'investigating', status: 'started',
    evidenceCount: allEvidence.length,
    totalClusters: allClusters.length,
    investigatingClusters: l1Clusters.length,
    skippedHighQuality: skipClusters.length,
    skippedByBudget: skippedByBudget.length,
    maxWorkers,
    pathwayBreakdown: countByType(allEvidence)
  });

  // Build synthetic evidence items for each cluster
  const clusterItems = l1Clusters.map(buildClusterEvidence);

  // Run cluster investigations with bounded concurrency
  ...
  // Map cluster results back to individual evidence items
  const pathwayResults = [];
  // Create a result map: clusterId -> pathway result
  const clusterResultMap = new Map();

  for (let i = 0; i < results.length; i++) {
    const result = results[i];
    const cluster = l1Clusters[i];
    const pr = result.status === 'fulfilled'
      ? result.value
      : {
          pathwayId: null, results: [],
          confidence: { confidence: 'U', label: 'UNVERIFIED', rationale: `Cluster pathway failed: ${result.reason?.message}` },
          crossPathways: []
        };
    clusterResultMap.set(cluster.clusterId, pr);
  }

  // Build per-evidence-item results: investigated items get their cluster result,
  // skipped items get a pass-through result
  for (const item of allEvidence) {
    let matched = false;

    // Check investigated clusters
    for (const cluster of l1Clusters) {
      if (cluster.items.some(ci => ci.id === item.id)) {
        pathwayResults.push(clusterResultMap.get(cluster.clusterId));
        matched = true;
        break;
      }
    }
    ...
    if (!matched) {
      // Item was in a skipped cluster (high quality or over budget) — pass through
      const passRationale = isHighQuality(item)
        ? 'High-quality evidence (A/B source, 1/2 info) — skipped investigation'
        : 'Skipped due to investigation budget cap';
      pathwayResults.push({
        pathwayId: null, results: [],
        confidence: {
          confidence: isHighQuality(item) ? 'P' : 'U',
          label: isHighQuality(item) ? 'PLAUSIBLE' : 'UNVERIFIED',
          rationale: passRationale
        },
        crossPathways: []
      });
    }
  ...
  const summary = {
    ...
    totalClusters: allClusters.length,
    investigatedClusters: l1Clusters.length,
    skippedHighQuality: skipClusters.length,
    skippedByBudget: skippedByBudget.length,
    maxWorkers,
  ...
  emitEvent('phase', {
    ...
    totalPathways: l1Clusters.length,
    totalEvidence: allEvidence.length,
