...
async function spawnLevelWorker(opts) {
  ...
  const label = `RESEARCH: ${pathway.id} L${levelDepth} "${evidenceItem.description || evidenceItem.id}"`;
...
async function runInvestigationPhase(projectDir, manifests, emitEvent, opts) {
  ...
  // Run all evidence items through their pathways in parallel
  const pathwayPromises = allEvidence.map(item =>
    runPathway(projectDir, item, emitEvent, opts)
  );

  const results = await Promise.allSettled(pathwayPromises);
  ...
  // Handle cross-pathway spawning (run discovered pathways)
  ...
  if (crossPathwayItems.length > 0) {
    ...
    const crossResults = await Promise.allSettled(
      crossPathwayItems.map(item => runPathway(projectDir, item, emitEvent, opts))
    );
