...
const BASE_URL = 'http://localhost:38007';
...
async function spawn(template, label, projectPath, parentWorkerId, taskDescription) {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    const result = await request('POST', '/api/workers/spawn-from-template', {
      template,
      label,
      projectPath,
      parentWorkerId,
    ...
    if (typeof result === 'string') {
      try { return JSON.parse(result); } catch { /* not JSON */ }
    }

    // Check for rate limiting
    if (result?.error && /too many|rate limit/i.test(result.error)) {
      if (attempt < MAX_RETRIES) {
        const delay = BASE_DELAY * Math.pow(2, attempt); // exponential backoff
        console.log(`[strategos.spawn] rate limited, retrying in ${delay}ms (attempt ${attempt + 1}/${MAX_RETRIES})`);
